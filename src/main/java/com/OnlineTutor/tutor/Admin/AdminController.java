...
public class AdminController {
    ...
    public ResponseEntity<List<TutorModel>>gettutorlist(){return adminService.gettutorlist();}
...
public ResponseEntity<List<StreamsubDto>>getStreamSubDetails(@RequestParam Long streamId){
  ...
  //status
    ...
    public ResponseEntity<List<StatusModel>>getStatus(){return adminService.getStatus();}
    ...
    public ResponseEntity<?>updTutorStatus(@RequestParam Long tutorId,@RequestParam Long statusId){
        try {
            return adminService.updatetutorStatus(tutorId, statusId);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return new ResponseEntity<>("Something went wrong", HttpStatus.INTERNAL_SERVER_ERROR);
